"""
config.py v6 — Groq as main agent
"""
...
DEFAULT_MAIN_MODEL      = "llama-3.1-8b-instant"
DEFAULT_FALLBACK_MODEL  = "llama-3.1-8b-instant"
DEFAULT_EVALUATOR_MODEL = "llama-3.3-70b-versatile"


def get_ai_config() -> dict:
    """Main agent — Groq first, Anthropic fallback."""
    groq_key = os.environ.get("GROQ_API_KEY", "").strip()
    ant_key  = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    model    = os.environ.get("AI_MODEL", "").strip()

    if groq_key and re.match(_GROQ_PATTERN, groq_key):
        return {
            "provider":       "groq",
            "api_key":        groq_key,
    ...
    return {"valid": False, "error": "No valid API key — set GROQ_API_KEY"}


def get_groq_config() -> dict:
    """Evaluator config — Groq."""
