...
def execute_tool(name: str, inp: dict) -> str:
    ...
    try:
        ...
        return json.dumps({"error": str(e), "tool": name})        url = f"https://api.duckduckgo.com/?q={enc}&format=json&no_html=1&skip_disambig=1"
        req = urllib.request.Request(url, headers={"User-Agent": "CompeteIQ-Bot/4.0"})
        with urllib.request.urlopen(req, timeout=10) as r:
            data = json.loads(r.read().decode("utf-8"))
        results = []
...
def brave_search(query: str, max_results: int = 5) -> dict:
    ...
    Falls back to DuckDuckGo if BRAVE_API_KEY not set.
    Get free key: api.search.brave.com (2000 searches/month free)
    """
    import os
    ...
    try:
        enc = urllib.parse.quote(query)
        url = f"https://api.search.brave.com/res/v1/web/search?q={enc}&count={max_results}&text_decorations=false"
        ...
        # Apply Metadata Filtering then Re-ranking
        results = _filter_by_metadata(results)
        results = _rerank_results(query, results)
